...
export async function openIfNeeded() {
  if (auction) return auction;
  if (!(await db.topPendingBid())) return null;
  const now = Date.now();
...
export async function noteBid() {
  const a = await openIfNeeded();
  if (!a) return null;
...
export async function takeWinner() {
  if (!closed()) return null;
  auction = null;
  return (await db.topPendingBid()) ?? null;
}

export async function status() {
  const pending = await db.pendingBids(50);
  const leader = pending[0] ?? null;
  return {
    open: !!auction,
