...
export const hideClip = async (id, reason = null) =>
  usingPg ? pg.hideClip(id, reason) : jsonStore.removeClip(id);
export const unhideClip = async (id) => (usingPg ? pg.unhideClip(id) : false);
...
export const reclaimStaleBids = async (min = 30) => (usingPg ? pg.reclaimStaleBids(min) : 0);
export const queuePosition = async (id) => (usingPg ? pg.queuePosition(id) : null);
...
// --- stats ------------------------------------------------------------------
export const hiddenClips = async (n = 50) => (usingPg ? pg.hiddenClips(n) : []);
export const generatedCount = async () =>
  usingPg ? pg.generatedCount()
          : jsonStore.state.clips.filter((c) => ['auto', 'bid', 'ad'].includes(c.source)).length;
export const stats = async () => (usingPg ? pg.stats() : jsonStore.state.stats);
export const paidSecondsToday = async () =>
  usingPg ? pg.paidSecondsToday()
          : jsonStore.state.clips.filter((c) => ['auto','bid','ad'].includes(c.source) && !c.mock)
              .reduce((n, c) => n + Number(c.duration || 0), 0);
...
// --- feedback ---------------------------------------------------------------
export const addFeedback = async (f) => {
  if (usingPg) return pg.addFeedback(f);
  console.log('[feedback]', f.kind, f.message.slice(0, 200));
  return { id: 'local', created_at: new Date() };
};
export const openFeedback = async (n = 50) => (usingPg ? pg.openFeedback(n) : []);
