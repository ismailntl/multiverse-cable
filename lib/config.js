...
export const config = {
  ...
  // Amazon Bedrock (Nova Reel) — the one good-quality backend that bills to
  // AWS credits instead of cash. $0.08 per second of output.
  bedrockEnabled: process.env.BEDROCK === '1',
  bedrockRegion: process.env.BEDROCK_REGION || 'us-east-1',
  // Bedrock goes through the AWS CLI and needs a profile; S3 uses the
  // long-lived IAM keys via the SDK, so the two auth paths are separate.
  bedrockProfile: process.env.BEDROCK_PROFILE || process.env.AWS_PROFILE || '',
  bedrockModel: process.env.BEDROCK_MODEL || 'amazon.nova-reel-v1:1',
  // Nova runs on AWS credits, so background clips can be long. fal is billed
  // per second in cash and stays at its 15s single-render cap.
  bedrockDuration: int(process.env.BEDROCK_DURATION_SEC, 0),

  // How many clips a hosted backend may render at once. Bedrock serialises
  // internally (Nova Reel's async quota is small); fal handles real parallelism.
  hostedConcurrency: int(process.env.HOSTED_CONCURRENCY, 4),
  // Keep generating until the AI library reaches this many clips. Survives
  // restarts, unlike a queued batch counter.
  aiTarget: int(process.env.AI_TARGET, 0),
  // Restrict automatic generation to a single genre. Set to 'live-shopping' to
  // make the channel produce only shopping segments and ads; unset generates
  // across every genre by weight. Viewer bids still choose their own genre.
  genreLock: process.env.GENRE_LOCK || '',
  // Review mode: newly generated clips land hidden instead of going straight to
  // air, so a new model's output can be judged before the public sees it.
  reviewMode: process.env.REVIEW_MODE === '1',
  // Slots reserved for paying bidders, kept clear of background seeding.
  paidConcurrency: int(process.env.PAID_CONCURRENCY, 2),

  falKey: process.env.FAL_KEY || '',
  // H3 Max Turbo: 2x the speed of H3 Max at a quarter the price ($0.01/s
  // promotional, $0.04/s after 2026-09-07, vs $0.08/s for plain H3 Max) while
  // holding ~97th percentile of its quality. Still returns synchronized audio,
  // which is the reason this backend exists. Single renders remain capped at
  // 15s; anything longer is stitched from segments in generate.js.
  falModel: process.env.FAL_MODEL || 'minimax/h3-max-turbo/image-to-video',
  falExpansion: process.env.FAL_EXPANSION || 'balanced',
  falResolution: (() => {
    const raw = (process.env.FAL_RESOLUTION || '768P').trim().toUpperCase();
    return ['480P', '768P'].includes(raw) ? raw : '768P';
  })(),
  ...
  videoDuration: int(process.env.VIDEO_DURATION_SEC, 10), // H3 accepts 4-15s
  ...
  // Hard money ceiling per UTC day. dailyGenLimit caps clip COUNT, which says
  // nothing about spend: 200 six-second clips is $12 on turbo and $96 on plain
  // H3 Max. This caps the thing that actually runs out.
  dailySpendUsd: Number(process.env.DAILY_SPEND_USD ?? 5),
  // What a second of generated video costs on the active paid backend, used to
  // estimate spend before committing to a render.
  falUsdPerSecond: Number(process.env.FAL_USD_PER_SECOND ?? 0.01),
  dailyGenLimit: int(process.env.DAILY_GEN_LIMIT, 200),
  // Only archival footage is ever pruned, and generated clips are unbounded, so
  // this cap mostly decides how fast the archive gets eaten. It was 500, which
  // meant every new AI clip deleted a real one.
  maxLibraryClips: int(process.env.MAX_LIBRARY_CLIPS, 3000),
  // "Real or AI?" only works while there is real footage to guess about. Never
  // prune the archive below this, whatever the cap says.
  archiveFloor: int(process.env.ARCHIVE_FLOOR, 120),
  ...
  cdnBase: (() => {
    ...
    if (!raw) return '';
    return raw.replace(/^([A-Za-z]+):\/\/([^/]+)/, (_, scheme, host) =>
