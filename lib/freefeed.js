...
// Prelinger = ~10k explicitly public-domain ephemeral films (the good stuff:
// industrial films, ads, PSAs — perfect found-footage texture). classic_cartoons
// is small (~81 items) but on-genre.
const COLLECTIONS = ['prelinger', 'prelinger', 'prelinger', 'classic_cartoons'];
const ROWS = 50;
...
async function randomItem() {
  ...
  // Skip anything whose title trips the minors/content filter
  const safe = docs.filter((d) => hardCheck(`${d.title ?? ''}`).allowed);
  if (!safe.length) throw new Error('no safe docs on this page');
