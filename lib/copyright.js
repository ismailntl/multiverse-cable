...
// Characters whose FILMS may be public domain but whose CHARACTER + name are
// still live trademarks (King Features, Fleischer, Disney, WB...). "Public
// domain" is a copyright status; it is not a trademark licence, and airing
// these on a monetized channel invites a trademark claim even when the print
// itself is free to copy. Applied to archival footage titles, not just prompts.
const TRADEMARKED_CHARACTERS = [
  'betty boop', 'popeye', 'olive oyl', 'bluto', 'superman', 'mickey', 'minnie',
  'donald duck', 'goofy', 'pluto the pup', 'bugs bunny', 'daffy', 'porky pig',
  'elmer fudd', 'looney', 'merrie melodies', 'silly symphon', 'woody woodpecker',
  'felix the cat', 'casper', 'little lulu', 'mighty mouse', 'tom and jerry',
  'gulliver', 'koko the clown', 'bimbo the dog', 'big bad wolf', 'three little pigs',
  'snow white', 'pinocchio', 'fleischer', 'walt disney', 'warner bros',
  'metro-goldwyn', 'paramount cartoon', 'terrytoons', 'ub iwerks',
];

export function trademarkedCharacterHit(text) {
  return hit(text, TRADEMARKED_CHARACTERS);
}
...
export function copyrightCheck(text) {
  const character = trademarkedCharacterHit(text);
  if (character) {
    return {
      allowed: false,
      reason: `"${character}" is a trademarked character — public-domain films don't grant trademark rights. Use an original character instead`,
    };
  }
