...
export const GENRES = [
  ...
  },
  {
    ...
    weight: 2,
    style:
      'A modern live shopping stream, the kind that runs on YouTube and TikTok: ' +
      'a bright ring-lit adult host talking straight down the lens at a phone camera, ' +
      'the product held up close and rotated, an on-screen price burst, a ticking ' +
      'countdown, a dwindling stock counter, and live chat scrolling up one side. ' +
      'The host reacts to the chat in real time and keeps insisting the deal is ending.',
  },
  {
    key: 'news',
...
export function conceptFromBid(bid) {
  // An ad bid can request the live-shopping treatment explicitly; otherwise it
  // still defaults to the infomercial look.
  const genre = pickGenre(
    bid.kind === 'ad' ? (bid.genre === 'live-shopping' ? 'live-shopping' : 'infomercial') : bid.genre
  );
  if (bid.kind === 'ad') {
...
const TITLE_TAIL = new Set([
  ...
  'only', 'your', 'their', 'its', 'one', 'where', 'when',
]);
...
export function templatePrompt(concept) {
  if (concept.genre === 'live-shopping') {
    return (
      ...
      `The adult host demonstrates the product to a phone camera and talks continuously. ` +
