...
async function s3() {
  ...
  // Explicit keys win over AWS_PROFILE. A dev box's SSO profile name got copied
  // into the production environment alongside good keys; the SDK then tried to
  // read a ~/.aws/config that does not exist on managed hosting and failed with
  // CredentialsProviderError while holding perfectly valid credentials. Every
  // upload threw for hours because of it.
  const key = process.env.AWS_ACCESS_KEY_ID;
  const secret = process.env.AWS_SECRET_ACCESS_KEY;
  const explicit = key && secret
    ? {
        credentials: {
          accessKeyId: key,
  ...
  client = new S3Client({ region: config.s3Region, ...explicit });
  return client;
}
