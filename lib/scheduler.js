...
import { randomConcept, conceptFromBid, writePrompt, safetySuffixFor } from './shows.js';
import { moderate } from './moderation.js';
import * as auction from './auction.js';
import * as playout from './playout.js';
import { s3Enabled, credentialStatus } from './storage.js';
...
// Refuse to generate when object storage is configured but unusable.
//
// This is not a nicety. On 2026-09-03 the host lost its AWS credentials, every
// upload threw, and the scheduler cheerfully spent ~$4.56 generating 31 clips
// straight onto an ephemeral disk that then wiped them. Paying to make videos
// nobody can ever watch is worse than not making them, so this fails closed.
let storageOk = null;
let storageCheckedAt = 0;

async function storageUsable() {
  if (!s3Enabled()) return true;             // local-disk mode is a valid setup
  if (storageOk !== null && Date.now() - storageCheckedAt < 60_000) return storageOk;
  const st = await credentialStatus();
  storageCheckedAt = Date.now();
  if (storageOk !== false && !st.resolved) {
    console.error(
      `[scheduler] HALTING GENERATION - object storage is configured but unusable ` +
        `(${st.reason ?? 'credentials did not resolve'}). Clips would be paid for and lost. ` +
        `Fix AWS credentials or unset S3_BUCKET to serve from local disk.`
    );
  }
  storageOk = st.resolved;
  return storageOk;
}

// Stop once today's generation has cost more than the ceiling. Only the paid
// hosted backend is metered -- a self-hosted GPU is already sunk cost.
let spentWarned = false;
async function withinSpendCap() {
  if (!config.dailySpendUsd) return true;
  if (!['fal', 'minimax'].includes(activeBackend())) return true;
  const spent = (await db.paidSecondsToday()) * config.falUsdPerSecond;
  if (spent < config.dailySpendUsd) { spentWarned = false; return true; }
  if (!spentWarned) {
    console.warn(
      `[scheduler] daily spend cap reached: ~$${spent.toFixed(2)} of ` +
        `$${config.dailySpendUsd.toFixed(2)}. Coasting on the library until UTC midnight.`
    );
    spentWarned = true;
  }
  return false;
...
async function runOne() {
  if (!(await storageUsable())) return 'skipped';
  // A paid bid is money coming in, so it is never blocked by the spend ceiling.
  if (!(await auction.takeWinner()) && !(await withinSpendCap())) return 'skipped';
  ...
  try {
    const prompt = (await writePrompt(concept)) + safetySuffixFor(concept);
    console.log(
    ...
    const clip = await db.addClip({
      ...
      // A paying bidder's clip always airs; only background seeding is held back
      // for review, so review mode can't silently swallow something bought.
      hidden: config.reviewMode && !bid,
...
// Durable seeding. A requested batch lives in memory, so a restart used to
// silently drop 200 queued clips. Instead of a counter, aim at a target: while
// the AI library is below AI_TARGET, the idle tick generates back-to-back. That
// survives restarts, self-limits, and still respects the daily spend cap.
async function seedingBehind() {
  if (!config.aiTarget) return false;
  try {
    // Must count hidden clips too — in review mode clips() excludes them, so
    // this would never reach target and would seed without end.
    return (await db.generatedCount()) < config.aiTarget;
  } catch {
    return false;
  }
}
...
export function startScheduler() {
  ...
  // Catch-up loop: while the AI library is short of target, keep a couple of
  // generations in flight instead of waiting out the idle interval.
  setInterval(async () => {
    if (batchRemaining > 0) return;
    if (!(await seedingBehind())) return;
    const room = Math.max(0, config.hostedConcurrency - inFlight);
    for (let i = 0; i < room; i += 1) runOne();
  }, 20_000);
}
