...
async function nextJob() {
  ...
  // A won bid is claimed immediately so a parallel worker can't take it too
  if (bid) store.settleBid(bid.id, 'generating');
...
async function runOne() {
  ...
  try {
    ...
    if (bid) {
      store.settleBid(bid.id, 'aired', clip.id);
