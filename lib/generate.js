...
async function falGenerate(prompt, file, seconds, imageUrl = null) {
  if (seconds <= FAL_MAX_SEGMENT_S) return falGenerateOne(prompt, file, seconds, imageUrl);
  return falGenerateLong(prompt, file, seconds, imageUrl);
}
...
async function falGenerateLong(prompt, file, seconds, imageUrl = null) {
  ...
  try {
    ...
    await Promise.all(
      Array.from({ length: count }, (_, i) => {
        ...
        parts[i] = seg;
        // Only the opening shot is conditioned on the product image; later
        // shots continuing the same scene would otherwise reset to it.
        return falGenerateOne(prompt + hint, seg, each, i === 0 ? imageUrl : null);
      })
    );
...
async function falGenerateOne(prompt, file, seconds, imageUrl = null) {
  const submitted = await falFetch(`https://queue.fal.run/${config.falModel}`, {
    ...
    body: JSON.stringify({
      ...
      // The configured model is an image-to-video endpoint. Given a product
      // image it animates that product; without one it still accepts the
      // request and generates from the prompt alone.
...
export async function generateClip(concept, prompt, durationSec, worker = null, opts = {}) {
  ...
  else if (backend === 'fal') await falGenerate(prompt, full, seconds, concept?.productImage ?? null);
