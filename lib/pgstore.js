...
export async function addClip(clip) {
  const row = await one(
    `insert into clips (file, title, channel, duration, genre, source, is_ad, prompt, archive_id, bidder, amount, mock, url, hidden)
    ...
    [clip.file, clip.title, clip.channel, clip.duration, clip.genre ?? null, clip.source,
     ...
     clip.amount ?? null, !!clip.mock, clip.url ?? null, !!clip.hidden]
  );
  ...
  // A clip with no URL 404s for every viewer, so never let that pass quietly.
  if (!s3Enabled() && !mapped.url) {
    console.warn(
      `[storage] NO OBJECT STORAGE CONFIGURED - "${mapped.title}" is only on this box's ` +
        `disk and will 404 for viewers. Set S3_BUCKET (and AWS credentials) in the environment.`
    );
  }
  if (s3Enabled() && !mapped.url) {
    try {
      ...
      console.error(
        `[storage] UPLOAD FAILED for ${mapped.file} (${mapped.title}): ${err.name}: ${err.message}`
      );
    }
    // A clip with no stored file 404s for every viewer, and on managed hosting
    // the local copy does not survive the next deploy either. Keep it off air
    // rather than letting it become a black screen in the rotation.
    if (!mapped.url) {
      await q(
        "update clips set hidden = true, hidden_reason = 'upload failed - no stored file' where id = $1",
        [mapped.id]
      );
      mapped = { ...mapped, hidden: true };
      console.error(`[storage] "${mapped.title}" held off air - it has no stored file`);
      return mapped;
    }
  }
...
async function pruneClips() {
  let over = clipCache.length - config.maxLibraryClips;
  if (over <= 0) return;
  // The archive is the only prunable source, so an unbounded generated library
  // would grind it to zero -- and with no real footage left, the "Real or AI?"
  // game has no real answers. Keep a floor of it no matter what.
  const archived = clipCache.filter((c) => c.source === 'archive').length;
  over = Math.min(over, Math.max(0, archived - config.archiveFloor));
  if (over <= 0) return;
...
// Counts generated clips INCLUDING hidden ones. Seeding targets must use this,
// not clips(), which excludes hidden — otherwise a review run never reaches its
// target and generates forever.
// Clips held back by review mode, newest first.
export async function hiddenClips(n = 50) {
  return (await q(
    "select * from clips where hidden and source in ('auto','bid','ad') order by created_at desc limit $1",
    [n]
  )).map(mapClip);
}
...
export async function generatedCount() {
  const row = await one(
    "select count(*)::int as n from clips where source in ('auto','bid','ad')"
  );
  return row?.n ?? 0;
}
...
// Seconds of paid video generated today, for the daily spend ceiling.
export async function paidSecondsToday() {
  const row = await one(
    "select coalesce(sum(duration),0)::numeric s from clips " +
    "where created_at::date = current_date and source in ('auto','bid','ad') and not mock"
  );
